...
The request is a [hotspot=28 file=1]`GET` HTTP request with the following header:

|===
| *Request Header* | *Request Value*
| Origin | The value is set to [hotspot=8 hotspot=26 file=2]`openliberty.io`.
|===
...
| Access-Control-Allow-Origin | Indicates whether the response can be shared with requesting code from the given origin (server name). The response value is expected to be `openliberty.io`.
| Access-Control-Allow-Credentials | Indicates whether the response can be exposed to the frontend code. The response value is expected to be [hotspot=15 file=0]`true`.
| Access-Control-Expose-Headers | Indicates the HTTP Headers that can be exposed. The response value is expected to be [hotspot=16 file=0]`MyHeader`.
|===
...
[source, java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/cors/TestData.java[tags=*]
----
...
|===
| *Request Header* | *Request Value*
| Origin | Server name of where a fetch originates from. `anywebsiteyoulike.com`
...
|===
| *Response Header* | *Response Value*
| Access-Control-Allow-Max-Age | Indicates number of seconds that the preflight request can be cached. The response value is expected to be [hotspot=23 file=0]`10`.
| Access-Control-Allow-Origin | Indicates whether the response can be shared with requesting code from the given origin. The response value is expected to be `anywebsiteyoulike.com`.
| Access-Control-Allow-Methods | Indicates method(s) allowed when accessing a resource in response to a preflight request. The response value is expected to be [hotspot=20 file=0]`OPTIONS, DELETE`.
| Access-Control-Allow-Credentials | Indicates whether the response can be exposed to the frontend code. The response value is expected to be [hotspot=21 file=0]`true`.
| Access-Control-Allow-Headers | Indicates HTTP headers that can be used during actual request. The response value is expected to be [hotspot=22 file=0]`MyOwnHeader1, MyOwnHeader2`.
|===
...
After you are done, stop the Open Liberty server:
[role='command']
----
