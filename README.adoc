...
Add a simple CORS configuration to the [hotspot=1-29]`server.xml` file in the `start/main/liberty/config/` directory:
...
Now we will test the simple CORS configuration that you added.

[role="code_command hotspot=25-33 file=1", subs="quotes"]
----
#Add the `testSimpleCorsRequest` method to the `TestCors` class.#
...
Add another CORS configuration in the [hotspot=1-29 file=0]`server.xml` file in the `start/main/liberty/config/` directory:
...
CORS configurations to allow two kinds of CORS requests in the [hotspot=1-29]`server.xml` file.
