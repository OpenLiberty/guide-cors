...
Add a simple CORS configuration to the [hotspot=*]`server.xml` file in the `start/main/liberty/config/` directory:

[role="code_command hotspot=12-16 file=0", subs="quotes"]
----
...
Save the changes to the [hotspot=1-29 file=0]`server.xml` file. The `/configurations/simple` endpoint is now ready to be
tested with a simple CORS request.

Open [hotspot=13-62 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory.

Now we will test the simple CORS configuration that you added. Add the [hotspot25-33]`testSimpleCorsRequest` method to the `TestCors` class.

[role="code_command hotspot=25-33 file=1", subs="quotes"]
----
...
[role="code_command hotspot=18-23 file=0", subs="quotes"]
----
#Replace the server configuration file.#
`src/main/liberty/config/server.xml`
----
...
Save the changes to the [hotspot=1-29 file=0]`server.xml` file. The `/configurations/preflight` endpoint is now ready to
be tested with a preflight CORS request.

Add another test to the [hotspot=13-62 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory to test the preflight CORS configuration that you just added:

[role="code_command hotspot=36-44 file=1", subs="quotes"]
----
#Add the `testPreflightCorsRequest` method to the `TestCors` class.#
`src/test/java/it/io/openliberty/guides/cors/TestCors.java`
----
