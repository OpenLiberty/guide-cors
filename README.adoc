...
| Access-Control-Expose-Headers | Indicates which headers can be exposed. The value is set to [hotspot=16 file=0]`MyHeader`.
|===
...
| [hotspot=19 file=0]`allowedOrigins`| The value is set to an asterisk (*) to allow requests from all origins.
| [hotspot=20 file=0]`allowedMethods`| The value is set to `OPTIONS, DELETE`.
| [hotspot=21 file=0]`allowCredentials`| The value is set to `true`.
|===
...
| Access-Control-Request-Method | Indicates which HTTP method will be used when the actual request is made. The value is set to [hotspot=20 file=0]`DELETE`.
| Access-Control-Request-Headers | Indicates which HTTP headers will be used when the actual request is made. The value is set to [hotspot=22 file=0]`MyOwnHeader2`
|===
...
|===
| *Header* | *Value*
| Access-Control-Allow-Max-Age | Indicates time in seconds that the preflight request can be cached. The value is set to [hotspot=23 file=0]`10`.
...
| Access-Control-Allow-Methods | Indicates method(s) allowed when accessing a resource in rsponse to a preflight request. The values are set to [hotspot=20 file=0]`OPTIONS, DELETE`.
...
| Access-Control-Allow-Headers | Indicates which HTTP headers can be used during actual request. The values are set to [hotspot=22 file=0]`MyOwnHeader1, MyOwnHeader2`.
|===
