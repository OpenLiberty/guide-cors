...
| Access-Control-Allow-Credentials | The expected value is [hotspot=15 file=0]`true`. Indicates that the user credentials can be included in the request.
| Access-Control-Expose-Headers |  The expected value is [hotspot=16 file=0]`MyHeader`. Indicates that the header `MyHeader` is safe to expose.
|===
