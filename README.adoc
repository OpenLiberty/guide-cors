...
Open [hotspot=25-78 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory.

Now we will test the simple CORS configuration that you added.

[role="code_command hotspot=25-78 file=1", subs="quotes"]
----
...
The [hotspot=26-34 file=1]`testSimpleCorsRequest` test simulates a client. It first sends a simple CORS request to the `/configurations/simple` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.
...
The [hotspot=26-34 file=1]`testSimpleCorsRequest` test passes and prints the response headers with their values from the endpoint. The
...
Add another test to the [hotspot=25-78 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory to test the preflight CORS configuration that you just added:
...
The request is an [hotspot=42 file=1]`OPTIONS` HTTP request with the following headers:
