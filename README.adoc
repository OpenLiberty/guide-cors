...
`src/main/java/io/openliberty/guides/cors/` directory.
...
Add a simple CORS configuration to the [hotspot file=0]`server.xml` file in the `src/main/liberty/config/` directory:
...
|===
| *Header* | *Value*
|[hotspot=12 file=0]`domain` | The endpoint to be configured for CORS requests. Set the value to `/configurations/simple`.
|[hotspot=13 file=0]`allowedOrigins` | Origins that are allowed to access the endpoint. Set the value to `openliberty.io`.
|[hotspot=14 file=0]`allowedMethods` | HTTP methods that a client is allowed to use when it makes requests to the endpoint. Set the value to `GET`.
|[hotspot=15 file=0]`allowCredentials` | A boolean that indicates whether the user credentials can be included in the request. Set the value to `true`.
|[hotspot=16 file=0]`exposeHeaders` | Headers that are safe to expose to clients. Set the value to `MyHeader`.
|===
...
Open [hotspot file=1]`TestCors.java` file in the `src/test/java/it/io/openliberty/guides/cors/` directory.
...
| Access-Control-Allow-Credentials | [hotspot=15 file=0]`true`
| Access-Control-Expose-Headers | [hotspot=16 file=0]`MyHeader`
|===
...
Configure the server to allow the `/configurations/preflight` endpoint to accept a [hotspot=1-29 file=0]`preflight` CORS request.
...
|===
| *Header* | *Value*
| [hotspot=18 file=0]`domain`|Set the value to `/configurations/preflight` because the `domain` is a different endpoint.
| [hotspot=19 file=0]`allowedOrigins`| Set the value to an asterisk (*) to allow requests from all origins.
| [hotspot=20 file=0]`allowedMethods`| Set the value to `OPTIONS, DELETE`.
| [hotspot=21 file=0]`allowCredentials`| Set the value to `true`.
|===

The follow attributes were added:

* [hotspot=23 file=0]`maxAge`:: The number of seconds that a client can cache a response to a preflight request. Set the value to [hotspot=23 file=0]`10`.
* [hotspot=22 file=0]`allowedHeaders`:: Headers that a client can use in requests. Set the value to [hotspot=22 file=0]`MyOwnHeader1, MyOwnHeader2`.
...
Add another test to the [hotspot file=1]`TestCors.java` file in the `src/test/java/it/io/openliberty/guides/cors/` directory to test the preflight CORS configuration that you just added:

[role="code_command hotspot=36-44 file=1", subs="quotes"]
----
#Replace the `TestCors` class.#
`src/test/java/it/io/openliberty/guides/cors/TestCors.java`
----
