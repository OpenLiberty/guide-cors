...
:page-seo-title: Enabling Cross-Origin Resource Sharing (CORS) in a REST service
...
|[hotspot=simple-domain file=0]`domain` | The endpoint to be configured for CORS requests. The value is set to `/configurations/simple`.
|[hotspot=simple-allowedOrigins file=0]`allowedOrigins` | Origins that are allowed to access the endpoint. The value is set to `openliberty.io`.
|[hotspot=simple-allowedMethods file=0]`allowedMethods` | HTTP methods that a client is allowed to use when it makes requests to the endpoint. The value is set to `GET`.
|[hotspot=simple-allowCredentials file=0]`allowCredentials` | A boolean that indicates whether the user credentials can be included in the request. The value is set to `true`.
|[hotspot=simple-exposeHeaders file=0]`exposeHeaders` | Headers that are safe to expose to clients. The value is set to `MyHeader`.
|===
...
Now we will test the simple CORS configuration that you added. Add the [hotspot=testSimpleCorsRequest file=1]`testSimpleCorsRequest` method to the `CorsIT` class.
...
The request is a [hotspot=testSimpleCorsRequest-get file=1]`GET` HTTP request with the following header:
...
|===
| *Configuration Attribute* | *Value*
| [hotspot=preflight-domain file=0]`domain`|The value is set to `/configurations/preflight` because the `domain` is a different endpoint.
| [hotspot=preflight-allowedOrigins file=0]`allowedOrigins`| Origins that are allowed to access the endpoint. The value is set to an asterisk (*) to allow requests from all origins.
| [hotspot=preflight-allowedMethods file=0]`allowedMethods`| HTTP methods that a client is allowed to use when it makes requests to the endpoint. The value is set to `OPTIONS, DELETE`.
| [hotspot=preflight-allowCredentials file=0]`allowCredentials`| A boolean that indicates whether the user credentials can be included in the request. The value is set to `true`.
|===

The follow attributes were added:

* [hotspot=preflight-maxAge file=0]`maxAge`: The number of seconds that a client can cache a response to a preflight request. Set the value to `10`.
* [hotspot=preflight-allowedHeaders file=0]`allowedHeaders`: Headers that a client can use in requests. Set the value to `MyOwnHeader1, MyOwnHeader2`.
...
The request is an [hotspot=testPreflightCorsRequest-options file=1]`OPTIONS` HTTP request with the following headers:
