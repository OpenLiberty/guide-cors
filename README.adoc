...
[role="code_command hotspot=1-29 file=0", subs="quotes"]
----
...
[hotspot=13 file=0]`domain`:: The endpoint to be configured for CORS requests. Set the value to `/configurations/simple`.
[hotspot=14 file=0]`allowedOrigins`:: Origins that are allowed to access the endpoint. Set the value to `openliberty.io`.
[hotspot=15 file=0]`allowedMethods`:: HTTP methods that a client is allowed to use when it makes requests to the endpoint. Set the value to `GET`.
[hotspot=16 file=0]`allowCredentials`:: A boolean that indicates whether the user credentials can be included in the request. Set the value to `true`.
[hotspot=17 file=0]`exposeHeaders`:: Headers that are safe to expose to clients. Set the value to `MyHeader`.
...
----
include::finish/src/test/java/it/io/openliberty/guides/cors/TestCors.java[tags=**]
----

The [hotspot=38-46 file=1]`testSimpleCorsRequest` test simulates a client. It first sends a simple CORS request to the `/configurations/simple` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.
...
The [hotspot=38-46 file=1]`testSimpleCorsRequest` test passes and prints the response headers with their values from the endpoint. The
...
[hotspot=21 file=0]`domain`:: Set the value to `/configurations/preflight` because the `domain` is a different endpoint.
[hotspot=22 file=0]`allowedOrigins`:: Set the value to an asterisk (*) to allow requests from all origins.
[hotspot=23 file=0]`allowedMethods`:: Set the value to `OPTIONS, DELETE`.
[hotspot=24 file=0]`allowCredentials`:: Set the value to `true`.

Add two new attributes:

[hotspot=26 file=0]`maxAge`:: The number of seconds that a client can cache a response to a preflight request. Set the value to [hotspot=26 file=0]`10`.
[hotspot=25 file=0]`allowedHeaders`:: Headers that a client can use in requests. Set the value to [hotspot=25 file=0]`MyOwnHeader1, MyOwnHeader2`.
...
The [hotspot=51-59 file=1]`testPreflightCorsRequest` test simulates a client sending a preflight CORS request. It first sends the request to the `/configurations/preflight` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.

The request is an [hotspot=54 file=1]`OPTIONS` HTTP request with the following headers:

|===
| *Header* | *Value*
| Origin | anywebsiteyoulike.com
| Access-Control-Request-Method | [hotspot=23 file=0]`DELETE`
| Access-Control-Request-Headers | [hotspot=25 file=0]`MyOwnHeader2`
|===
...
|===
| *Header* | *Value*
| Access-Control-Allow-Max-Age | [hotspot=26 file=0]`10`
| Access-Control-Allow-Origin | anywebsiteyoulike.com
| Access-Control-Allow-Methods | [hotspot=23 file=0]`OPTIONS, DELETE`
| Access-Control-Allow-Credentials | [hotspot=24 file=0]`true`
| Access-Control-Allow-Headers | [hotspot=25 file=0]`MyOwnHeader1, MyOwnHeader2`
|===
...
The [hotspot=51-59]`testPreflightCorsRequest` test passes and prints the response headers with their values from the endpoint.
