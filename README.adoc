...
Open [hotspot=15-64 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory.

Now we will test the simple CORS configuration that you added.

[role="code_command hotspot=27-35 file=1", subs="quotes"]
----
Edit the `TestCors` class
`src/test/java/it/io/openliberty/guides/cors/TestCors`
----
TestCors.java
...
The [hotspot=27-35 file=1]`testSimpleCorsRequest` test simulates a client. It first sends a simple CORS request to the `/configurations/simple` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.
...
The [hotspot=27-35 file=1]`testSimpleCorsRequest` test passes and prints the response headers with their values from the endpoint. The
...
Add another test to the [hotspot=15-64 file=1]`TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory to test the preflight CORS configuration that you just added:


[role="code_command hotspot=39-47 file=1", subs="quotes"]
----
Edit the `TestCors` class
`src/test/java/it/io/openliberty/guides/cors/TestCors`
----

The [hotspot=39-47 file=1]`testPreflightCorsRequest` test simulates a client sending a preflight CORS request. It first sends the request to the `/configurations/preflight` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.

The request is an [hotspot=41 file=1]`OPTIONS` HTTP request with the following headers:

|===
| *Header* | *Value*
| Origin | anywebsiteyoulike.com
| Access-Control-Request-Method | [hotspot=23 file=0]`DELETE`
...
The [hotspot=39-47 file=1]`testPreflightCorsRequest` test passes and prints the response headers with their values from the endpoint.
