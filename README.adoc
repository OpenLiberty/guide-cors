...
|===
| *Request Header* | *Request Value*
| Origin | The value is set to [hotspot=8 hotspot=26 file=2]`openliberty.io`. Indicates that the request originates from `openliberty.io`.
|===
...
| Access-Control-Allow-Origin | The expected value is `openliberty.io`. Indicates whether the response can be shared with requesting code from the given origin (server name). 
| Access-Control-Allow-Credentials | The expected value is [hotspot=15 file=0]`true`. Indicates whether the response can be exposed to the frontend code. 
| Access-Control-Expose-Headers |  The expected value is [hotspot=16 file=0]`MyHeader`. Indicates the HTTP Headers that can be exposed.
|===
...
| Access-Control-Request-Method | The value is set to [hotspot=20 file=0]`DELETE`. Indicates that the HTTP DELETE method will be used in the actual request.
| Access-Control-Request-Headers | The value is set to [hotspot=22 file=0]`MyOwnHeader2`. Indicates the header `MyOwnHeader2` will be used in the actual request.
|===
...
|===
| *Response Header* | *Response Value*
| Access-Control-Allow-Max-Age | The expected value is [hotspot=23 file=0]`10`. Indicates that the preflight request can be cached within `10` seconds.
| Access-Control-Allow-Origin | The expected value is `anywebsiteyoulike.com`. Indicates whether a resource can be shared based one the returning value of the Origin request header `anywebsiteyoulike.com`.
| Access-Control-Allow-Methods | The expected value is [hotspot=20 file=0]`OPTIONS, DELETE`.  Indicates that HTTP OPTIONS and DELETE methods can be used in the actual request.
| Access-Control-Allow-Credentials | The expected value is [hotspot=21 file=0]`true`. Indicates that the user credentials can be included in the request.
| Access-Control-Allow-Headers | The expected value is [hotspot=22 file=0]`MyOwnHeader1, MyOwnHeader2`. Indicates that the header `MyOwnHeader1` and `MyOwnHeader2` are safe to expose.
|===
