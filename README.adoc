...
[hotspot=13]`domain`:: The endpoint to be configured for CORS requests. Set the value to `/configurations/simple`.
[hotspot=14]`allowedOrigins`:: Origins that are allowed to access the endpoint. Set the value to `openliberty.io`.
[hotspot=15]`allowedMethods`:: HTTP methods that a client is allowed to use when it makes requests to the endpoint. Set the value to `GET`.
[hotspot=16]`allowCredentials`:: A boolean that indicates whether the user credentials can be included in the request. Set the value to `true`.
[hotspot=17]`exposeHeaders`:: Headers that are safe to expose to clients. Set the value to `MyHeader`.

Save the changes to the [hotspot=1-29]`server.xml` file. The `/configurations/simple` endpoint is now ready to be
tested with a simple CORS request.

Open `TestCors.java` file in the `start/src/test/java/it/io/openliberty/guides/cors/` directory.

Now we will test the simple CORS configuration that you added.

[role="code_command hotspot=25-78", subs="quotes"]
----
...
The [hotspot=38-46]`testSimpleCorsRequest` test simulates a client. It first sends a simple CORS request to the `/configurations/simple` endpoint, and then it checks for a valid response and expected headers. Lastly, it prints the response headers for you to inspect.
...
The [hotspot=38-46]`testSimpleCorsRequest` test passes and prints the response headers with their values from the endpoint. The
