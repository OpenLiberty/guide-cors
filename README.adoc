...
CORS is useful for requesting many kinds of data from websites other than your own, such as images, videos, scripts, style sheets, iFrames, and web fonts.

However, you cannot request resources from another website's domain without proper permission. In JavaScript, cross-origin requests with an `XMLHttpRequest` API and Ajax cannot happen unless CORS is enabled on the server that is receiving the request. Otherwise, same-origin security policy prevents the requests. For example, a web page that is served from the `\http://aboutcors.com` server sends a request to get data to the
...
If the [hotspot=testSimpleCorsRequest file=1]`testSimpleCorsRequest` test passes, it will print the response headers, with their values, from the endpoint. The
...
If the [hotspot=testPreflightCorsRequest file=1]`testPreflightCorsRequest` test passes, it will print the response headers, with their values, from the endpoint.
...
Feel free to modify the server configuration and the test code to experiment with the
